...
public:
    ...
    Vector3f normalized () const { float inv_norm = 1.0f / sqrt(x * x + y * y + z * z);
                                   return Vector3f(x * inv_norm, y * inv_norm, z * inv_norm); }
    inline float dot (const Vector3f &v) const { return x * v.x + y * v.y + z * v.z; }
    inline Vector3f cross (const Vector3f &v) const { return Vector3f(y * v.z - z * v.y, 
                                                                      z * v.x - x * v.z,
                                                                      x * v.y - y * v.x); }
