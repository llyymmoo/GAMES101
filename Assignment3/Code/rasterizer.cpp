    ...
    for (int i = min_x; i <= max_x; ++i)
    {
        for (int j = min_y; j <= max_y; ++j)
        {
            for (int k = 0; k < sample_num; ++k)
            {
                ...
                {
                    // shader
                    fragment_shader_payload payload(get_interpolated_color(pixel, t),
                                                    get_interpolated_normal(pixel, t).normalized(),
                                                    get_interpolated_texcoords(pixel, t),
                                                    texture ? &*texture : nullptr);
                    payload.view_pos = get_interpolated_viewpos(pixel, t, view_pos);
                    Eigen::Vector3f pixel_color = fragment_shader(payload);

                    // set frame buf
                    Eigen::Vector3f point = Eigen::Vector3f(i, j, k);
                    Eigen::Vector3f tmp = set_super_pixel(point, pixel_color); 
                    super_depth_buf[ind] = depth;

                    int index = get_index(i, j);
                    frame_buf[index] = (frame_buf[index] * sample_num - tmp + pixel_color) / sample_num;
                }
            }
        }
    }
...
float rst::rasterizer::get_triangle_depth(std::pair<float, float> pixel, const Triangle& t)
{
    ...
    float z_interpolated = alpha * t.z[0] + beta * t.z[1] + gamma * t.z[2];

    return -z_interpolated;
}

Eigen::Vector3f rst::rasterizer::get_interpolated_color(std::pair<float, float> pixel, const Triangle& t)
{
...
Eigen::Vector3f rst::rasterizer::get_interpolated_normal(std::pair<float, float> pixel, const Triangle& t)
{
...
Eigen::Vector2f rst::rasterizer::get_interpolated_texcoords(std::pair<float, float> pixel, const Triangle& t)
{
    ...
    return interpolated_texcoords;
}

Eigen::Vector3f rst::rasterizer::get_interpolated_viewpos(std::pair<float, float> pixel, const Triangle& t, const std::array<Eigen::Vector3f, 3>& view_pos)
{
    Eigen::Vector3f interpolated_viewpos;
    auto[alpha, beta, gamma] = computeBarycentric2D(pixel.first, pixel.second, t.v);
    interpolated_viewpos = alpha * view_pos[0] + beta * view_pos[1] + gamma * view_pos[2];
    
    return interpolated_viewpos;
}
...
rst::rasterizer::rasterizer(int w, int h, int sample_points) : width(w), height(h), sample_num(sample_points)
{
    frame_buf.resize(w * h);
    depth_buf.resize(w * h);
