...
namespace rst
{
    ...
    public:
        ...
        rasterizer(int w, int h, int sample_points);
        ...
        bool is_inside_triangle(std::pair<float, float> pixel, const Triangle& t);

        void set_pixel(const Vector2i &point, const Eigen::Vector3f &color);

        Eigen::Vector3f set_super_pixel(const Eigen::Vector3f& point, const Eigen::Vector3f& color);
        float get_triangle_depth(std::pair<float, float> pixel, const Triangle& t);
        Eigen::Vector3f get_interpolated_color(std::pair<float, float> pixel, const Triangle& t);
        Eigen::Vector3f get_interpolated_normal(std::pair<float, float> pixel, const Triangle& t);
        Eigen::Vector2f get_interpolated_texcoords(std::pair<float, float> pixel, const Triangle& t);
        Eigen::Vector3f get_interpolated_viewpos(std::pair<float, float> pixel, const Triangle& t, const std::array<Eigen::Vector3f, 3>& view_pos);
