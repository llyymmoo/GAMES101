...
{
public:
    MeshTriangle(const std::string& filename, const std::string& splitMethod)
    {
        objl::Loader loader;
        ...
        else if (splitMethod == "BVH")
            bvh = new BVHAccel(ptrs, 1, BVHAccel::SplitMethod::NAIVE);
        else
            std::cout << "<error> : no such SplitMethod method ! pls choose from SAH or BVH !" << std::endl;
    }
